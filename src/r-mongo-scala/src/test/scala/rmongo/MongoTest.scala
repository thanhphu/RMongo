...
class MongoTest {
	...
	def testDbReplicaSetInsertDocument() {
		...
		val errMsg =
			"""Write failed with error code 11000 and error message 'E11000 duplicate key
			  |error collection: test.test_data index: _id_ dup key: { : "foo" }'""".stripMargin
	...
	def testToCsvOutput() {
		...
		rMongo.dbInsertDocument("test_data", doc)
